...
The instruction is correct — but only for the direct dependencies. The `require` block forglet writes contains only the dependencies declared in `.forglet.yml`. Nothing else.

After forglet writes `go.mod`, the developer runs `go mod tidy`. The Go toolchain analyses the full dependency graph and appends a second `require` block containing all indirect (transitive) dependencies:
...
That block is not in `.forglet.yml`. Forglet does not know it exists. It was written by `go mod tidy` and only `go mod tidy` should touch it.

The result is a file with two owners and no boundary drawn between them. The marker claims the whole file. A developer who manually edits the indirect block and then runs `forglet synth` will have their edits silently erased — forglet rewrites the file from scratch, and the indirect block is gone until `go mod tidy` is run again.

The correct mental model: forglet owns the shape of `go.mod` (module path, Go version, direct dependencies). The Go toolchain owns the indirect dependencies. The two are partners in a required sequence — `forglet synth` then `go mod tidy` — and the marker currently does not communicate this split.

**Open questions:**
- Should the marker text name both owners? e.g. `// ~~ module path, go version, and direct deps managed by forglet; indirect deps managed by go mod tidy ~~`
- Should `forglet synth` automatically run `go mod tidy` for Go projects, or at minimum print a reminder?

*Discovered during the first live `forglet synth` run on the vergant project.*
